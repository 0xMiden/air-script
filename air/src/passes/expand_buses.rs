...
use crate::{Air, BusBoundary, CompileError, ConstraintDomain, Operation, TraceAccess};
...
impl Pass for BusOpExpand<'_> {
    ...
    fn run<'a>(&mut self, mut ir: Self::Input<'a>) -> Result<Self::Output<'a>, Self::Error> {
        ...
        for (bus_index, (_ident, bus)) in buses.iter().enumerate() {
            ...
            let bus_trace_access = TraceAccess::new(1, bus_index, 0);
            let bus_trace_access_with_offset = TraceAccess::new(1, bus_index, 1);
            ...
            match bus_type {
                BusType::Multiset => {
                    let root;
                    {
                        ...
                        for bus_op in bus_ops {
                            ...
                            for (col_index, column) in columns.iter().enumerate() {
                                ...
                                let alpha = graph.insert_node(Operation::Value(
                                    crate::Value::RandomValue(col_index + 1),
                                ));

                                // 1.3 Multiply arg with alpha
                                let arg_times_alpha =
                                    graph.insert_node(Operation::Mul(*column, alpha));

                                // 1.4 Combine with other args
                                args_combined = graph
                                    .insert_node(Operation::Add(args_combined, arg_times_alpha));
                            }
                            ...
                            let one =
                                graph.insert_node(Operation::Value(crate::Value::Constant(1)));
                            let inverse_latch = graph.insert_node(Operation::Sub(one, latch));
                            let args_combined_with_latch_and_latch_inverse = graph.insert_node(
                                Operation::Add(args_combined_with_latch, inverse_latch),
                            );
                            ...
                            match bus_op_kind {
                                ...
                                BusOpKind::Remove => {
                                    p_prime_factor = match p_prime_factor {
                                        Some(p_prime_factor) => {
                                            Some(graph.insert_node(Operation::Mul(
                                                p_prime_factor,
                                                args_combined_with_latch_and_latch_inverse,
                                            )))
                                        }
                        ...
                        let p_prod = match p_factor {
                            Some(p_factor) => {
                                graph.insert_node(Operation::Mul(p_factor, bus_access))
                            }
                            None => bus_access,
                        };
                        let p_prime_prod = match p_prime_factor {
                            Some(p_prime_factor) => graph.insert_node(Operation::Mul(
                                p_prime_factor,
                    ...
                    ir.constraints
                        .insert_constraint(1, root, ConstraintDomain::EveryRow);
                }
                BusType::Logup => {
                    let root;
                    {
                        ...
                        for (bus_index, bus_op) in bus.bus_ops.iter().enumerate() {
                            ...
                            for (i, factor) in factors.iter().enumerate() {
                                if i != bus_index {
                                    factors_without_current = match factors_without_current {
                                        Some(factors_without_current) => Some(graph.insert_node(
                                            Operation::Mul(factors_without_current, *factor),
                                        )),
                                        None => Some(*factor),
                                    };
                                }
                            ...
                            let factors_without_current_with_latch = match factors_without_current {
                                Some(factors_without_current) => graph
                                    .insert_node(Operation::Mul(factors_without_current, latch)),
                                None => latch,
                            };
                        ...
                        let q_prod = match total_factors {
                            Some(total_factors) => {
                                graph.insert_node(Operation::Mul(total_factors, bus_access))
                            }
                            None => bus_access,
                        };
                        let q_prime_prod = match total_factors {
                            Some(total_factors) => graph
                                .insert_node(Operation::Mul(total_factors, bus_access_with_offset)),
                            None => bus_access_with_offset,
                        };
                        ...
                        let q_prime_term = match terms_removed_from_bus {
                            Some(terms_removed_from_bus) => graph
                                .insert_node(Operation::Add(q_prime_prod, terms_removed_from_bus)),
                            None => q_prime_prod,
                        };
    ...
    fn handle_boundary_constraint(
        ...
        match bus_boundary {
            ...
            BusBoundary::Null => {
                ...
                ir.constraints.insert_constraint(1, root, domain);
            }
        }
    }
}
