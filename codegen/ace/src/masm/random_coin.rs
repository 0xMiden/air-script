use crate::masm::{MasmVerifierParameters, ProofOptions};

/// Generates the MASM module of the STARK verifier for managing the random coin
/// i.e., the Fiat-Shamir transcript.
pub fn generate_random_coin_module(
    masm_verifier_parameters: &MasmVerifierParameters,
    proof_options: &ProofOptions,
) -> String {
    let main_segment_width = masm_verifier_parameters.main_trace_width();
    let (aux_segment_width, num_aux_segments) =
        if let Some(aux_trace_width) = masm_verifier_parameters.aux_trace_width() {
            (aux_trace_width, 1)
        } else {
            (0, 0)
        };
    let num_aux_random_values = masm_verifier_parameters.num_auxiliary_randomness();
    let fri_remainder_poly_max_degree = proof_options.fri_remainder_max_degree();
    ...
    RANDOM_COIN_FILE
        ...
        .replace(
            "{MAIN_SEGMENT_WIDTH}",
            &format!("{main_segment_width:02x}",),
        )
        .replace("{AUX_SEGMENT_WIDTH}", &format!("{aux_segment_width:02x}",))
        .replace("{NUM_AUX_SEGMENTS}", &format!("{num_aux_segments:02x}",))
        .replace(
            "{NUM_AUX_RANDOM_ELEMENTS}",
            &format!("{num_aux_random_values:02x}",),
        )
        .replace(
            "{FIELD_EXTENSION_DEGREE}",
            &format!("{:02x}", proof_options.field_extension_degree()),
        )
        .replace(
            "{FRI_FOLDING_FACTOR}",
            &format!("{:02x}", proof_options.fri_folding_factor()),
        )
        .replace(
            "{FRI_REMAINDER_POLY_MAX_DEGREE}",
            &format!("{fri_remainder_poly_max_degree:02x}",),
        )
        .replace(
            "{BLOWUP_FACTOR}",
            &format!("{:02x}", proof_options.blowup_factor()),
        )
}

const RANDOM_COIN_FILE: &str = r#"
...
export.init_seed
    ...
    ## main segment width is 0x{MAIN_SEGMENT_WIDTH} and there are 0x{NUM_AUX_SEGMENTS} auxiliary segments
    ## of width 0x{AUX_SEGMENT_WIDTH} using 0x{NUM_AUX_RANDOM_ELEMENTS} random extension field elements
    push.0x{MAIN_SEGMENT_WIDTH}{NUM_AUX_SEGMENTS}{AUX_SEGMENT_WIDTH}{NUM_AUX_RANDOM_ELEMENTS}
    swap
    ...
    push.0x{FIELD_EXTENSION_DEGREE}{FRI_FOLDING_FACTOR}{FRI_REMAINDER_POLY_MAX_DEGREE}{BLOWUP_FACTOR}
